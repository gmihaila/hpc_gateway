...
def jupyter_instance(_euid, _pass, hostname, session_timeout):
    ...
    is_user, is_jupyter_config, jupyter_sha, jupyter_last_port = jupyter_port(
        euid=_euid, passw=_pass, addrs=hostname, timeout=5)
    ...
    if is_user and is_jupyter_config and jupyter_last_port == "no_ports":
        ...
        if jupyter_last_port and (jupyter_last_port != "no_ports"):
            ...
            forward_port(euid=_euid, passw=_pass, addrs=hostname, remote_port=jupyter_last_port,
                         local_port=jupyter_last_port, timeout=session_timeout)
        ...
        print("COMPLETE FAIL!")
    print("\nis_user: %s \nis_jupyter_config: %s \njupyter_sha: %s \njupyter_last_port: %s" % (
        is_user, is_jupyter_config, jupyter_sha, jupyter_last_port))
    return
...
jupyter_instance(_euid=euid, _pass=pass_, hostname=addrs,
